...
prostate (PRAD), colon (COAD)**. It reaches **~89% accuracy** across all five (classical baseline
...
**Result (trained to convergence, 32 volumes × 16 epochs, ~50 min on M5 GPU):**
**val WT 0.860 · TC 0.794 · ET 0.734**: on par with the 2-D model on whole-tumor and **better on
tumor core** (0.794 vs 0.712). On a fully held-out case it reaches **TC 0.915 · ET 0.877**:

<p align="center">
  <img src="assets/unet3d_prediction.png" alt="3-D U-Net predicted tumor" width="360">
</p>

The volumetric model sees through-plane context, so it's especially strong on the inner tumor
sub-regions. Shipped as `models/unet3d_brats.pt`. (Earlier this model was undertrained from an
interrupted run; trained to completion it is now competitive with the 2-D model, and better on the
core/enhancing sub-regions.)
